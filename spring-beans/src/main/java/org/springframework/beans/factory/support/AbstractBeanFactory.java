...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
	...
	protected <T> T doGetBean(
		...
		// name prefix is “&” gain factoryBean instance
		String beanName = transformedBeanName(name);
		Object bean;

		// 从单例缓存中或者FactoryBean获取 Bean 对象
		...
		if (sharedInstance != null && args == null) {
			...
			* */
			...
			// <4> 如果bean definition容器中没有找到，且父BeanFactory不为空，则从父类容器中加载
			BeanFactory parentBeanFactory = getParentBeanFactory();
			if (parentBeanFactory != null && !containsBeanDefinition(beanName)) {
				// Not found -> check parent.
				// 定义原始bean name
				// 先去除beanName前缀所有的&，再加上一个&
			...
			try {
				...
				if (dependsOn != null) {
					for (String dep : dependsOn) {
						// 若给定的依赖 dep(被依赖的beanName) 已经注册到beanName其它依赖bean中
						/*
	...
	protected String transformedBeanName(String name) {
		/*
		* #transformedBeanName()：如果 name 以 “&” 为前缀，那么会去掉该 "&" ；会无线循环的去除 &
		* #canonicalName():通过请求参数去别名容器aliasMap中寻找对应的beanName
		* */
	...
	protected RootBeanDefinition getMergedLocalBeanDefinition(String beanName) throws BeansException {
		...
		//<XXX> 如果返回的 BeanDefinition 是子类 bean 的话，则合并父类相关属性
	...
	protected Object getObjectForBeanInstance(
		...
		if (object == null) {
			...
			// <XXX> 核心处理方法，使用 FactoryBean 获得 Bean 对象
